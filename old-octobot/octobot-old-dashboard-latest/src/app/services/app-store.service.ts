...
export class AppStoreService {
  public requestHasChanged$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  public hasOpenAppsSidebar$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  // http: Http;
  TOKEN: string;
  ...
  constructor(
    ...
    this.APPS_URL =
      environment.production && environment.production === true
        ? this.APPS_BASE_URL + "api/apps?sort=score"
        : this.APPS_BASE_URL;
  ...
  getInstallation(projectId) {
    let promise = new Promise((resolve, reject) => {
      ...
      this._httpClient
        .get(this.APPS_BASE_URL + "api/installation/" + projectId, {
          headers: headers,
        })
        .toPromise()
        .then((res) => {
  ...
  unistallNewApp(projectId: string, appId: string) {
    let url =
      this.APPS_BASE_URL + "api/installation/" + projectId + "/" + appId;
