  ...
  getChatUrl() {
    ...
    let jwtData =
      environment.production && environment.production === true
        ? ""
        : `?jwt=${localStorage.getItem("tiledesk_token")}`;


        this.CHAT_BASE_URL = this.appConfigService.getConfig().CHAT_BASE_URL + jwtData

    this.CHAT_BASE_URL2 = this.appConfigService.getConfig().CHAT_BASE_URL2;
